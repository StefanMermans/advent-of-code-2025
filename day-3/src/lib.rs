...
pub fn total_joltage(args: Args) -> Result<i64, Box<dyn Error>> {
    let batteries_to_turn_on = batteries_to_turn_on(&args);
    
    Ok(fs::read_to_string(args.input)?
        .split('\n')
        .filter(|bank|!bank.is_empty())
        .map(|bank|bank_joltage(bank, batteries_to_turn_on).unwrap())
        .sum::<i64>())
}

fn bank_joltage(bank: &str, remaining: usize) -> Result<i64, ParseIntError> {
